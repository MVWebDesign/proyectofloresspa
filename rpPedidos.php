...
<body>
    ...
    <div class="report-title">
        <h2>Descargar Reporte de Pedidos (Archivo PDF)</h2>
    </div>
    ...
    <div class="container reportButtons">
        <div class="report-divide">
            ...
            <div class="form-group">
                <select class="form-control selected" required name="nombreP" placeholder="Proveedor">
                    ...
                    while ($row = $consulta->fetch_array()) {
                        echo "<option value='".$row["FechaEntrega"]."'>".$row["FechaEntrega"]."</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="form-group">
                <select class="form-control selected" required name="nombreP" placeholder="Proveedor">
                    <?php
                    $consulta = $conexion->query("SELECT * FROM tpedidos ORDER BY id DESC");
                    while ($row = $consulta->fetch_array()) {
                        echo "<option value='".$row["FechaEntrega"]."'>".$row["FechaEntrega"]."</option>";
                    }
                    ?>
